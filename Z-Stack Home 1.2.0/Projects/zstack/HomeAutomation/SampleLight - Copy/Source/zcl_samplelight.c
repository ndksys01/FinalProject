...
void zclSampleLight_Init( byte task_id )
{
  ...
  //osal_set_event(zclSampleLight_TaskID,TIME_SEND_ACCE_EVT);
  //osal_set_event(zclSampleLight_TaskID,TIME_SEND_LIGH_EVT);
  
  zclSampleLight_DstAddr.addrMode = (afAddrMode_t)Addr16Bit;
  zclSampleLight_DstAddr.endPoint = 0x010;       //end point 20 dia chi muon gui
  zclSampleLight_DstAddr.addr.shortAddr = 0xFFFF;

  zclSampleLight_epDesc.endPoint = 0x010;
  zclSampleLight_epDesc.task_id = &zclSampleLight_TaskID;
  zclSampleLight_epDesc.simpleDesc
            = (SimpleDescriptionFormat_t *)&zclSampleLight_SimpleDesc;
 zclSampleLight_epDesc.latencyReq = noLatencyReqs;
...
#ifdef ZCL_LEVEL_CTRL
    zclSampleLight_LevelCurrentLevel = zclSampleLight_OnOff ? zclSampleLight_LevelOnLevel : 0;
#endif
  }
...
static void zclSampleLight_DisplayLight( void )
{
  ...
  if (giLightScreenMode == LIGHT_MAINMODE)
  {
    ...
    if ( zclSampleLight_LevelCurrentLevel == 0 )
    {
      HalLcdWriteString( (char *)sLightOff, HAL_LCD_LINE_2 );
    }
    else if ( zclSampleLight_LevelCurrentLevel >= 254 )
    {
...
static void zclSampleLight_DefaultMove( void )
{
  ...
  // calculate rate based on time (int 10ths) for full transition (0-255)
  time = zclSampleLight_GetTime( newLevel, 0xffff );
...
static void zclSampleLight_AdjustLightLevel( void )
{
  ...
  {
    if ( zclSampleLight_LevelCurrentLevel )
    {
      zclSampleLight_OnOff = LIGHT_ON;
    }
    else
...
static void zclSampleLight_LevelControlMoveCB( zclLCMove_t *pCmd )
{
  ...
  {
    newLevel = ATTR_LEVEL_ON;  // fully on
  }
  else
  {
    newLevel = ATTR_LEVEL_OFF; // fully off
  }
...
static void zclSampleLight_LevelControlStepCB( zclLCStep_t *pCmd )
{
  ...
  {
    if ( (uint16)zclSampleLight_LevelCurrentLevel + pCmd->amount > ATTR_LEVEL_ON )
    {
      newLevel = ATTR_LEVEL_ON;
    }
    else
    {
    ...
    if(*pBuf =='#')
      {
         ...
         case '1':
          ...
          case '2':
           {
             SendDataFull("#2",0x12,"Enable Security.");
             break;
           }
          case '3':
           {
             SendDataFull("#3",0x13,"Disable Security.");
             break;
           }
           case '4':
           {
           ...
           case '6':
           {
              ...
              //SendDataFull("#6",0x16,"Turning Off Light 1");
             break;
           }
           case '7':
           {
           ...
           case '9':
           {
             SendDataFull("#9",0x19,"Turning On Light 3");
             break;
           }
           case 'a':
           {
             SendDataFull("#a",0x1a,"Turning Off Light 3");
             break;
           }
           case 'b':
           {
             SendDataFull("#b",0x1b,"Turning On Light 4");
             break;
           }
           case 'c':
           {
             SendDataFull("#c",0x1c,"Turning On Light 4");
             break;
           }
           case 'd':
           {
              autoLight=1;
              SendDataFull("#d",0x1d,"Enable Auto Light");
              break;
           }
           case 'e':
           {
...
void SendData(int cluID)
 {
  ...
  if ( AF_DataRequest( &zclSampleLight_DstAddr, &zclSampleLight_epDesc,
                       cluID, //=1 tin nhan test
                       (byte)osal_strlen( data ) + 1,
                       (byte *)&data,
                       &zclSampleLight_TransID,
